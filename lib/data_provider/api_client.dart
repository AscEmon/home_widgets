...
class ApiClient {
  ...
  Future clientHandle(
    ...
    try {
       ...
       */
      _handleResponse(response: response, onSuccessFunction: onSuccessFunction);
  ...
  void _handleNoInternet({required APIParams apiParams}) {
        ...
        ViewUtil.showInternetDialog(
          onPressed: () {
            if (NetworkConnection.instance.isInternet == true) {
              Navigator.of(
                Navigation.key.currentState!.overlay!.context,
                rootNavigator: true,
              ).pop();
